...
const ProductDetail: React.FC = () => {
  ...
  if (loading) return <div className="min-h-screen flex items-center justify-center font-bold uppercase tracking-widest text-[#FF5F00] animate-pulse">Đang tải siêu phẩm...</div>;
      ...
      <section className="bg-white pt-20">
        <main className="max-w-7xl mx-auto px-4 md:px-8 py-6 md:py-10">
          ...
          <div className="flex items-center space-x-2 text-[9px] md:text-xs font-bold text-gray-400 uppercase tracking-widest mb-6 md:mb-8 overflow-x-auto whitespace-nowrap scrollbar-hide">
            <Link to="/" className="hover:text-[#FF5F00] transition-colors">Trang chủ</Link>
            <ChevronRight size={12} />
            <Link to={`/category/${product.category}`} className="hover:text-[#FF5F00] transition-colors">{product.category}</Link>
            <ChevronRight size={12} />
            <span className="text-[#FF5F00] truncate">{product.name}</span>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-16">
            ...
            <div className="space-y-4 md:space-y-6">
              <div className="relative aspect-square rounded-[2rem] md:rounded-[3rem] overflow-hidden border border-gray-100 shadow-xl md:shadow-2xl group bg-white">
                ...
                {discountPercentage > 0 && (
                  <div className="absolute top-4 left-4 md:top-6 md:left-6 bg-[#FF5F00] text-white px-3 py-1.5 md:px-4 md:py-2 rounded-xl md:rounded-2xl font-black text-xs md:text-sm shadow-xl">
                    -{discountPercentage}%
                  </div>
                )}
              </div>
              <div className="relative group/gallery">
                ...
                <button 
                  onClick={() => scroll('left')}
                  className="absolute left-0 top-1/2 -translate-y-1/2 z-10 bg-white/80 hover:bg-white p-1 rounded-full shadow-md group-hover/gallery:flex items-center justify-center transition-all border border-gray-100"
                >
                  <ChevronLeft size={16} />
                </button>
                ...
                >
                  {[product.mainImage, ...product.gallery].map((img, idx) => (
                    <button 
                      ...
                      className={`flex-shrink-0 w-16 h-20 md:w-20 md:h-24 rounded-xl md:rounded-2xl overflow-hidden border-2 transition-all ${activeImg === img ? 'border-[#FF5F00] scale-95 shadow-lg' : 'border-gray-50 hover:border-gray-200'}`}
                    >
                ...
                <button 
                  ...
                  className="absolute right-0 top-1/2 -translate-y-1/2 z-10 bg-white/80 hover:bg-white p-1 rounded-full shadow-md group-hover/gallery:flex items-center justify-center transition-all border border-gray-100"
                >
            ...
            <div className="flex flex-col space-y-6 md:space-y-8">
              <div className="space-y-3 md:space-y-4">
                <div className="inline-block px-3 py-1 bg-[#FF5F00]/10 text-[#FF5F00] rounded-full text-[8px] md:text-[10px] font-black uppercase tracking-widest border border-[#FF5F00]/20">{product.brand}</div>
              ...
              <div className="flex items-end space-x-3 md:space-x-4 py-4 md:py-6 border-y border-gray-50">
                <div className="text-3xl md:text-4xl font-black text-[#FF5F00]">{(product.salePrice || product.price).toLocaleString()}đ</div>
              ...
              <div className="space-y-6">
                      ...
                      <div className="space-y-3 md:space-y-4">
                        <div className="flex justify-between items-center">
                          ...
                          {(product.category === 'Quần áo' || product.category === 'Giày Thể Thao') && (
                            <button className="text-[9px] font-bold text-[#FF5F00] uppercase underline underline-offset-4">Quy đổi size</button>
                          )}
                        </div>
                        <div className="flex flex-wrap gap-2 md:gap-3">
                ...
                <div className="flex items-center space-x-6">
                  ...
                  <div className="flex items-center border-2 border-gray-100 rounded-xl md:rounded-2xl p-0.5 md:p-1 bg-gray-50">
                    <button onClick={() => setQuantity(Math.max(1, quantity - 1))} className="p-1.5 md:p-2 hover:text-[#FF5F00] transition-colors font-bold text-lg md:text-xl px-3">-</button>
                    <span className="w-10 md:w-12 text-center font-black text-sm md:text-lg">{quantity}</span>
                    <button onClick={() => setQuantity(quantity + 1)} className="p-1.5 md:p-2 hover:text-[#FF5F00] transition-colors font-bold text-lg md:text-xl px-3">+</button>
                  </div>
                </div>
                ...
                <div className="flex flex-col sm:flex-row gap-3 md:gap-4">
                  ...
                  <button 
                    ...
                    className="flex-1 bg-[#FF5F00] text-white py-4 md:py-5 rounded-xl md:rounded-[2rem] font-black uppercase tracking-widest text-[10px] md:text-xs flex items-center justify-center shadow-xl shadow-[#FF5F00]/20 hover:bg-black transition-all active:scale-95"
                  >
      ...
      <section className="bg-slate-50/50 py-12 md:py-24 border-y border-gray-100">
        <div className="max-w-7xl mx-auto px-4">
          <div className="flex justify-center space-x-6 md:space-x-12 mb-10 md:mb-16">
            <button onClick={() => setShowSpecs(false)} className={`pb-3 md:pb-4 text-[10px] md:text-xs font-black uppercase tracking-[0.2em] md:tracking-[0.4em] transition-all border-b-2 md:border-b-4 ${!showSpecs ? 'border-[#FF5F00] text-[#FF5F00]' : 'border-transparent text-gray-300 hover:text-gray-400'}`}>Mô tả</button>
            <button onClick={() => setShowSpecs(true)} className={`pb-3 md:pb-4 text-[10px] md:text-xs font-black uppercase tracking-[0.2em] md:tracking-[0.4em] transition-all border-b-2 md:border-b-4 ${showSpecs ? 'border-[#FF5F00] text-[#FF5F00]' : 'border-transparent text-gray-300 hover:text-gray-400'}`}>Thông số</button>
          </div>

          <div className="max-w-4xl mx-auto">
            {!showSpecs ? (
              <div className="relative bg-white p-6 md:p-16 rounded-[2rem] md:rounded-[3rem] shadow-sm border border-gray-100">
                <div className={`prose prose-orange max-w-none text-gray-600 leading-relaxed font-medium transition-all duration-700 overflow-hidden ${!isExpanded ? 'max-h-[400px] md:max-h-[500px]' : 'max-h-[10000px]'}`}>
                ...
                {!isExpanded && (
                  <div className="absolute bottom-0 left-0 right-0 h-32 md:h-48 bg-gradient-to-t from-white via-white/80 to-transparent flex items-end justify-center pb-6 md:pb-10 rounded-b-[2rem] md:rounded-b-[3rem]">
                    <button onClick={() => setIsExpanded(true)} className="bg-gray-950 text-white px-8 md:px-10 py-3 md:py-4 rounded-full font-black uppercase text-[9px] md:text-[10px] tracking-widest hover:bg-[#FF5F00] transition-all shadow-xl">Xem thêm</button>
                  </div>
                )}
                {isExpanded && <button onClick={() => setIsExpanded(false)} className="mt-8 md:mt-12 block mx-auto text-gray-400 font-black uppercase text-[9px] md:text-[10px] tracking-widest hover:text-[#FF5F00] transition-colors underline underline-offset-8">Thu gọn</button>}
              </div>
            ) : (
              <div className="bg-white rounded-[2rem] md:rounded-[3rem] border border-gray-100 shadow-xl overflow-hidden">
                <div className="bg-gray-950 p-4 md:p-6 flex items-center space-x-3">
                  <div className="w-1.5 h-1.5 md:w-2 md:h-2 bg-[#FF5F00] rounded-full animate-pulse" />
