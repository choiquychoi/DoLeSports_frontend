    ...
    <div className="min-h-screen bg-white font-sans text-gray-900">
      ...
      <main className="max-w-7xl mx-auto px-4 md:px-8 py-8 md:py-12">
        ...
        <div className="flex items-center space-x-2 text-[9px] md:text-[10px] font-black text-gray-400 uppercase tracking-widest mb-6 md:mb-8 overflow-x-auto whitespace-nowrap scrollbar-hide pb-1">
          ...
          <span className="text-[#FF5F00] font-black uppercase">{categoryName}</span>
        </div>
            ...
            <div className="bg-white lg:bg-transparent">
              <div className="flex justify-between items-center mb-4 md:mb-6">
                <h3 className="text-base md:text-lg font-black uppercase tracking-tighter italic flex items-center">
                  <Filter size={16} className="mr-2 text-[#FF5F00]" /> Bộ lọc
                </h3>
                {(brand || minPrice || maxPrice) && (
                  <button onClick={clearFilters} className="text-[9px] md:text-[10px] font-bold text-gray-400 hover:text-[#FF5F00] flex items-center uppercase">
              ...
              <div className="space-y-6">
                ...
                <div className="pb-6 lg:pb-8 border-b border-gray-100 lg:block">
                  ...
                  <div className="flex overflow-x-auto lg:flex-col gap-2 pb-2 lg:pb-0 no-scrollbar">
                    <button 
                      onClick={() => setBrand('')}
                      className={`px-3 py-1.5 md:px-4 md:py-2 rounded-lg md:rounded-xl text-[10px] md:text-xs font-bold text-left transition-all whitespace-nowrap ${brand === '' ? 'bg-[#FF5F00] text-white shadow-md' : 'bg-gray-50 text-gray-500 hover:bg-gray-100'}`}
                    >
                      Tất cả
                    </button>
                    {Array.isArray(availableBrands) && availableBrands.map((b) => (
                      <button 
                        ...
                        className={`px-3 py-1.5 md:px-4 md:py-2 rounded-lg md:rounded-xl text-[10px] md:text-xs font-bold text-left transition-all whitespace-nowrap ${brand === b ? 'bg-[#FF5F00] text-white shadow-md' : 'bg-gray-50 text-gray-500 hover:bg-gray-100'}`}
                      >
                        {b}
                      </button>
                    ))}
                ...
                <div className="pt-2 pb-6 lg:pb-8 lg:border-b border-gray-100">
                  ...
                  <form onSubmit={handlePriceApply} className="flex lg:flex-col gap-2 md:gap-3">
                    <input 
                      ...
                      className="w-full p-2.5 md:p-3 bg-gray-50 border border-transparent rounded-lg md:rounded-xl text-[10px] md:text-xs font-bold outline-none focus:border-[#FF5F00]"
          ...
          <div className="flex-1">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 md:mb-10 gap-4 md:gap-6">
              ...
              <div className="flex items-center space-x-2 md:space-x-3 bg-gray-50 p-1.5 md:p-2 rounded-xl md:rounded-2xl border border-gray-100 w-full md:w-auto">
                <div className="p-1.5 md:p-2 bg-white rounded-lg md:rounded-xl shadow-sm text-[#FF5F00]">
                  <ArrowUpDown size={14} />
                </div>
                <select 
                        ...
                        <Link to={`/product/${product.slug}`}>
                          <Card className="group overflow-hidden border-none shadow-sm hover:shadow-xl transition-all duration-300 bg-white cursor-pointer rounded-[1.5rem] md:rounded-[2.5rem]">
                            <CardContent className="p-0 relative aspect-[4/5] overflow-hidden bg-gray-50/50 m-2 md:m-4 rounded-xl md:rounded-[2rem]">
                              ...
                              {discount > 0 && <Badge className="absolute top-2 left-2 md:top-4 md:left-4 bg-[#FF5F00] font-black px-2 py-0.5 md:px-3 md:py-1 rounded-lg md:rounded-xl shadow-lg border-none text-white text-[8px] md:text-[10px]">-{discount}%</Badge>}
                            </CardContent>
                            <CardFooter className="flex flex-col items-center p-3 md:p-6 pt-0 md:pt-2 text-center">
                              <span className="text-[8px] md:text-[9px] font-black text-[#FF5F00] uppercase tracking-widest mb-0.5 md:mb-1">{product.brand}</span>
                              <h3 className="font-bold text-[10px] md:text-sm mb-2 md:mb-3 line-clamp-1 group-hover:text-[#FF5F00] transition-colors uppercase px-1">{product.name}</h3>
                ...
                <div className="flex items-center space-x-1.5 md:space-x-2">
                  {[...Array(pages).keys()].map((x) => (
                    <button
                      key={x + 1}
                      onClick={() => setPage(x + 1)}
                      className={`w-10 h-10 md:w-12 md:h-12 rounded-xl md:rounded-2xl font-black text-[10px] md:text-xs transition-all border ${page === x + 1 ? 'bg-[#FF5F00] text-white border-[#FF5F00] shadow-lg scale-105 md:scale-110' : 'bg-white text-gray-400 border-gray-100 hover:border-[#FF5F00]/30 hover:text-[#FF5F00]'}`}
                    >
                      {x + 1}
                    </button>
                  ))}
